---
name: release
description: Prepare, authorize, publish, troubleshoot, and verify Cursor Free desktop GitHub Releases. Use for version bumps, release tags, GitHub Actions release runs, updater manifests, signing, or release-readiness checks.
---
...
- Never print, commit, or upload `.tauri/cursor-free.key` anywhere except the repository's `TAURI_SIGNING_PRIVATE_KEY` Actions Secret when the author explicitly requests that secret configuration.
...
3. Confirm the updater public key in `tauri.conf.json` matches `.tauri/cursor-free.key.pub` without exposing the private key.
